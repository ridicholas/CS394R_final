...
def play(
    ...
    while game.running:
        ...
        if done:
            ...
            games.append((states, actions, rews))
            obs = env.reset()
...
def game():
    ...
    games = play(gym.make("CartPole-v1"), keys_to_action={(ord('w'), ): 0, (ord('d'), ): 1}, seed=random.randint(0,10000))
    i=1

    #all_states = []
    all_states = np.zeros((1,4))
    all_actions = []
    rewards = 0
    for game in games:

        all_states = np.vstack([all_states, game[0]])
        all_actions += game[1]
        rewards += len(game[1])
        print('game {}'.format(i))d
        print(' states: {}'.format(game[0]))
        print(' actions: {}'.format(game[1]))
        print(' rewards: {}'.format(game[2]))
        print(
            'num states: {}, num actions: {}, num rewards: {}'.format(game[0].shape[0], len(game[1]), sum(game[2])))

        i+=1
    all_states = all_states[1:, :]
    print('final num states: {}, num actions: {}, num rewards: {}'.format(all_states.shape[0], len(all_actions), rewards))

    s = pd.DataFrame(all_states)
    a = pd.DataFrame({'states': all_actions})
    s.to_csv('s.csv')
    a.to_csv('a.csv')
    r = pd.DataFrame({'r':np.array([rewards/len(games)])})
    r.to_csv('r.csv')

    return rewards/len(games), all_states, all_actions
